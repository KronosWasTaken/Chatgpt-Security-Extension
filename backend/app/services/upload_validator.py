...
Current behavior:
- DO NOT block on size, extension, MIME, or magic bytes (they only log).
- DO block uploads that match sensitive data patterns or sensitive extensions.
"""
import os
import json
import re
...
class UploadValidator:
    ...
    # --------------------
    # NON-BLOCKING SAFETY CHECKS (size/MIME/magic just log)
    # --------------------

    def validate_file_size(self, file_size: int) -> None:
        """
        Validate file size in a non-blocking way.
        Logs a warning if above configured limit but does NOT raise.
        """
        if file_size > self.max_upload_bytes:
            logger.warning(
                f"upload_size_exceeded size={file_size} max={self.max_upload_bytes} (NOT BLOCKING)"
            )
        # No exception -> always allow
    ...
    def validate_mime_type(self, mime_type: str) -> None:
        """
        Validate MIME type in a non-blocking way.
        Logs a warning if not in the configured allowlist but does NOT raise.
        """
        if not mime_type:
            logger.info("upload_mime_missing (NOT BLOCKING)")
        ...
        if mime_type not in self.allowed_mime_types:
            ...
            if not matched:
                logger.warning(
                    f'upload_mime_unexpected mime="{mime_type}" (NOT BLOCKING)'
                )
    ...
    def validate_magic_bytes(self, file_content: bytes, filename: str) -> None:
        """
        Validate magic bytes match declared type in a non-blocking way.
        Logs a warning on mismatch but does NOT raise.
        """
        ...
        if detected_type and declared_ext:
            ...
            # Mismatch for other types – log but don't block
            logger.warning(
                f'upload_magic_mismatch contentType={detected_type} declaredExt={declared_ext} (NOT BLOCKING)'
            )

    # --------------------
    # SENSITIVE DATA SCANNING (BLOCKING)
    # --------------------
    ...
    def validate_upload(
        ...
        Behavior:
        - Size, extension, MIME, and magic-byte checks are NON-BLOCKING (log only).
        - Sensitive files (by extension or pattern) are BLOCKED.
        ...
        corr_id = (log_context or {}).get('correlationId', 'unknown')

        # Log upload received
        logger.info(
            f"upload_received corrId={corr_id} "
            f"name={filename} size={file_size} mime={mime_type} ext={self.get_file_extension(filename)}"
        )

        # 1. Size check (non-blocking)
        self.validate_file_size(file_size)

        # 2. Extension check (non-blocking)
        self.validate_file_extension(filename)

        # 3. MIME check (non-blocking)
        self.validate_mime_type(mime_type or '')

        # 4. Magic byte verification (non-blocking)
        self.validate_magic_bytes(file_content, filename)

        # 5. Sensitive data scanning (BLOCKING)
        has_sensitive, match_reason = self.scan_for_sensitive_data(file_content, mime_type or '', filename)

        if has_sensitive:
            logger.warning(
                f"upload_sensitive_detected corrId={corr_id} "
                f"reason={match_reason} previewLen={self.max_preview}"
            )
            # Block sensitive files
            raise UploadValidationError(
                'SENSITIVE_CONTENT',
                match_reason or 'Sensitive data detected in upload'
            )

        # If we get here, file passed checks (no sensitive content)
        detected_from_magic = self.detect_file_type_from_magic(file_content)
        detected_mime = mime_type or 'application/octet-stream'

        # Best-effort: map magic type to MIME
        if detected_from_magic == 'pdf':
            detected_mime = 'application/pdf'
        elif detected_from_magic in ['jpg', 'jpeg']:
            detected_mime = 'image/jpeg'
        elif detected_from_magic == 'png':
            detected_mime = 'image/png'
        elif detected_from_magic == 'zip':
            detected_mime = 'application/zip'

        logger.info(
            f"upload_magic_verified corrId={corr_id} "
            f"detectedMime={detected_mime}"
        )
        logger.info(
            f"upload_accepted corrId={corr_id} "

...
from app.models import (
    Client, ClientAIServices, Alert, DepartmentEngagement, 
...
class MetricsCalculator:
    ...
    async def calculate_client_metrics(self, client_id: str, target_date: date = None) -> Dict[str, Any]:
        """Calculate all metrics for a specific client"""
        if target_date is None:
            target_date = date.today()
        
        # Calculate apps monitored (count of unique AI services)
        apps_monitored = await self._calculate_apps_monitored(client_id)
        
        # Calculate interactions monitored (sum of daily interactions)
        interactions_monitored = await self._calculate_interactions_monitored(client_id, target_date)
        ...
        risk_score = await self._calculate_risk_score(client_id, target_date)
    ...
    async def _calculate_apps_monitored(self, client_id: str) -> int:
        """Count unique AI services for the client"""
    ...
    async def _calculate_interactions_monitored(self, client_id: str, target_date: date) -> int:
        """Sum all daily interactions for the client"""
        # Get interactions from department engagement
        ...
        # Get interactions from application engagement
    ...
    async def calculate_department_engagement(self, client_id: str, target_date: date = None) -> List[Dict[str, Any]]:
        """Calculate department engagement metrics"""
        if target_date is None:
            target_date = date.today()
        
        # Get department data
        query = select(DepartmentEngagement).where(
            and_(
                DepartmentEngagement.client_id == client_id,
                DepartmentEngagement.date == target_date
            )
    ...
    async def calculate_application_engagement(self, client_id: str, target_date: date = None) -> List[Dict[str, Any]]:
        """Calculate application engagement metrics"""
        if target_date is None:
            target_date = date.today()
        
        # Get application data
        query = select(ApplicationEngagement).where(
            and_(
                ApplicationEngagement.client_id == client_id,
                ApplicationEngagement.date == target_date
            )
